...
for (int i = 0; i < N; ++i) {
  ...
  for (int j = i + 1; j < N; ++j) {
    ...
    for (int dim = 0; dim < 3; ++dim) { // baseline gradient
    ...
    if (can_diff) {
      for (int endpoint = 0; endpoint < 4; ++endpoint) {   // loop over the 4 endpoints: 0=a0, 1=a1, 2=b0, 3=b1
        for (int dim = 0; dim < 3; ++dim) {
          ...
          double da0[3] = {0,0,0}, da1[3] = {0,0,0}, db0[3] = {0,0,0}, db1[3] = {0,0,0}; // unit perturbation applied to one endpoint coordinate
          ...
          double dMt0 = dBB*u + (-dEE)*v; // dM * [u;v]
